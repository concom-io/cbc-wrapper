...
def install_cbc
  ...
  Dir.chdir "/tmp" do
    ...
    res = system "cd #{CBC_SRC_DIR} && ./configure --prefix=#{CBC_INSTALL} -C --with-pic --without-static --disable-shared --enable-cbc-parallel --enable-debug && make -j `bash -c \"grep -c ^processor /proc/cpuinfo\"` && make install"
    if not res
