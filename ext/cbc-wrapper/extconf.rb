...
def install_cbc
  ...
  Dir.chdir "/tmp" do
    ...
    res = system "cd #{CBC_SRC_DIR} && ./configure ADD_CFLAGS='-g -gdwarf-2 -O0' ADD_CXXFLAGS='-g -gdwarf-2 -O0' --prefix=#{CBC_INSTALL} -C --with-pic --without-static && make -j `bash -c \"grep -c ^processor /proc/cpuinfo\"` && make install"
    if not res
